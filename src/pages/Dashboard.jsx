...
function Heatmap({ studyLog }) {
  ...
  // Nomes dos meses para tooltip e labels
  const MESES = ['Jan','Fev','Mar','Abr','Mai','Jun','Jul','Ago','Set','Out','Nov','Dez'];

  return (
    <div>
      {/* Grade + painel direito lado a lado */}
      <div style={{ display:'flex', gap:8, alignItems:'flex-start' }}>
        ...
        <div style={{ flexShrink:0 }}>
          ...
          <div style={{ display:'flex', gap:2, marginBottom:3, paddingLeft:0 }}>
            {weeks.map((wk, wi) => {
              const d = new Date(wk[0].key + 'T00:00:00');
              const show = d.getDate() <= 7;
              return (
                <div key={wi} style={{ width:10, fontSize:'0.44rem', color: show ? '#475569' : 'transparent',
                    textTransform:'capitalize', whiteSpace:'nowrap', overflow:'visible' }}>
                  {show ? MESES[d.getMonth()] : ''}
                </div>
              );
            })}
          </div>
          ...
          <div style={{ display:'flex', gap:2 }}>
            {weeks.map((wk, wi) => (
              <div key={wi} style={{ display:'flex', flexDirection:'column', gap:2 }}>
                {wk.map((c, di) => (
                  <div key={di}
                    title={`${c.key}: ${c.min > 0 ? fmt(c.min) : 'sem estudo'}`}
                    style={{ width:10, height:10, borderRadius:2, background:col(c.min),
                      border: c.key===today ? '1px solid #818cf8' : 'none' }}/>
                ))}
              </div>
            ))}
          </div>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:2, marginTop:5 }}>
            <span style={{ fontSize:'0.46rem', color:'#334155' }}>menos</span>
            {['rgba(255,255,255,0.04)','rgba(99,102,241,0.25)','rgba(99,102,241,0.5)','rgba(99,102,241,0.75)','#6366f1'].map((bg,i) => (
              <div key={i} style={{ width:9, height:9, borderRadius:2, background:bg }}/>
            ))}
            <span style={{ fontSize:'0.46rem', color:'#334155' }}>mais</span>
          </div>
        </div>

        {/* Painel direito: stats empilhadas */}
        <div style={{ flex:1, display:'grid', gridTemplateColumns:'1fr 1fr', gap:'6px 10px', alignContent:'start' }}>
          {[
            ['🔥', 'Streak',    `${streak}d`,                                      '#f59e0b'],
            ['📅', 'Dias ativos', `${totalDias}`,                                  '#6366f1'],
            ['⏱',  'Total foco', fmt(totalMin),                                    '#22d3ee'],
            ['📊', 'Média/dia',  totalDias > 0 ? fmt(Math.round(totalMin/totalDias)) : '—', '#10b981'],
          ].map(([ic, label, val, color]) => (
            <div key={label} style={{ background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.05)',
                borderRadius:8, padding:'6px 8px' }}>
              <div style={{ display:'flex', alignItems:'center', gap:4, marginBottom:2 }}>
                <span style={{ fontSize:'0.75rem' }}>{ic}</span>
                <span style={{ fontSize:'0.52rem', color:'#334155', textTransform:'uppercase',
                    letterSpacing:'0.07em', fontWeight:700 }}>{label}</span>
              </div>
              <div style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:'0.88rem',
...
export default function Dashboard() {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
      ...
      {/* ── LINHA 2: Mapa de atividade + Pomodoro + Countdown + Evolução ── */}
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr', gap:10, alignItems:'stretch' }}>
        ...
        <Card style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', minHeight:200 }}>
          <ST>Countdown da Prova</ST>
          {countdown !== null && countdown > 0 ? (
            <div style={{ textAlign:'center', flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:6 }}>
              <div style={{ fontSize:'0.62rem', fontWeight:800, letterSpacing:'0.14em', textTransform:'uppercase',
                  color: countdown<=30?'#ef4444': countdown<=90?'#f59e0b':'#10b981', marginBottom:2 }}>
                FALTAM
              </div>
              <div style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:'3.8rem', fontWeight:700, lineHeight:1,
                  color: countdown<=30?'#ef4444': countdown<=90?'#f59e0b':'#10b981',
                  textShadow: countdown<=30?'0 0 20px rgba(239,68,68,0.35)': countdown<=90?'0 0 20px rgba(245,158,11,0.35)':'0 0 20px rgba(16,185,129,0.35)' }}>
                {countdown}
              </div>
              <div style={{ fontSize:'0.72rem', color:'#475569', fontWeight:600 }}>dias</div>
              {metas.nomeProva && (
                <div style={{ fontSize:'0.7rem', fontWeight:700, color:'#94a3b8', marginTop:4,
                    background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.07)',
                    borderRadius:8, padding:'4px 12px', maxWidth:'100%',
                    overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                  {metas.nomeProva}
                </div>
              )}
              {metas.dataProva && (
                <div style={{ fontSize:'0.62rem', color:'#334155', marginTop:2 }}>
                  {new Date(metas.dataProva+'T00:00:00').toLocaleDateString('pt-BR',{weekday:'short',day:'2-digit',month:'long',year:'numeric'})}
                </div>
              )}
              <div style={{ marginTop:8, width:'100%', height:4, background:'rgba(255,255,255,0.05)', borderRadius:99, overflow:'hidden' }}>
                <div style={{ height:'100%', borderRadius:99,
                    background: countdown<=30?'#ef4444': countdown<=90?'#f59e0b':'#10b981',
                    width:`${Math.max(2, Math.min(100, 100 - (countdown / 365) * 100))}%`,
                    transition:'width 0.4s' }}/>
              </div>
            </div>
          ) : (
            <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:8, color:'#1e293b' }}>
              <div style={{ fontSize:'2rem' }}>📅</div>
              <div style={{ fontSize:'0.72rem', textAlign:'center', lineHeight:1.5 }}>
                Configure a data da prova<br/>
                <span style={{ fontSize:'0.65rem', color:'#334155' }}>em Análise → Countdown</span>
              </div>
            </div>
          )}
        </Card>
